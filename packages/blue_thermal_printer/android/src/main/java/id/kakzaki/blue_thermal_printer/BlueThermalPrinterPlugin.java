...
public class BlueThermalPrinterPlugin implements FlutterPlugin, ActivityAware,MethodCallHandler, RequestPermissionsResultListener {
  ...
  public boolean onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
    ...
    if (requestCode == REQUEST_COARSE_LOCATION_PERMISSIONS || requestCode == REQUEST_BLUETOOTH_PERMISSIONS || requestCode == 1) {
      ...
      if (allGranted) {
        ...
        pendingResult.error("no_permissions", "bluetooth permissions are required for scanning paired devices", null);
      }
      pendingResult = null;
      return true;
    }
  ...
  private void emitRead(final byte[] bytes) {
    if (readSink == null) return;

    mainHandler.post(() -> {
      final EventSink sink = readSink;
      if (sink == null) return;
      try {
        sink.success(bytes);
      } catch (RuntimeException ex) {
        Log.w(TAG, "Dropping Bluetooth read event after sink detach", ex);
      }
    });
  }

  private void emitStatus(final Object status) {
    if (statusSink == null) return;

    mainHandler.post(() -> {
      final EventSink sink = statusSink;
      if (sink == null) return;
      try {
        sink.success(status);
      } catch (RuntimeException ex) {
        Log.w(TAG, "Dropping Bluetooth status event after sink detach", ex);
      }
    });
  }
    ...
    public void run() {
      ...
      while (true) {
        try {
          ...
          if (readSink != null && bytes > 0) {
            emitRead(Arrays.copyOf(buffer, bytes));
          }
  ...
  private final StreamHandler stateStreamHandler = new StreamHandler() {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
          THREAD = null;
          emitStatus(intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, -1));
    ...
    public void onCancel(Object o) {
      ...
      try {
        ...
        Log.w(TAG, "Bluetooth status receiver was already unregistered", ex);
      }
    }
  };
