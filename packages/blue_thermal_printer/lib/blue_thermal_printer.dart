...
class BlueThermalPrinter {
  ...
  ///onReadRaw()
  Stream<Uint8List> onReadRaw() => _readChannel.receiveBroadcastStream().map(
        (buffer) {
          if (buffer is Uint8List) return buffer;
          if (buffer is List) return Uint8List.fromList(buffer.cast<int>());
